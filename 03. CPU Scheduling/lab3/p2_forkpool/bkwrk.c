#include "bktpool.h"
#include <signal.h>
#include <stdio.h>

#define _GNU_SOURCE
#include <linux/sched.h>
#include <sys/syscall.h>      /* Definition of SYS_* constants */
#include <unistd.h>

//#define DEBUG
#define INFO
#define SHARED_MEMORY
//#define WORK_THREAD

void * bkwrk_worker(void * arg) {
  sigset_t set;
  int sig;
  int s;
  int i = * ((int * ) arg); // Default arg is integer of workid

  struct bkworker_t * wrk = & worker[i];
  wrk->wrkid = i;

  /* Taking the mask for waking up */
  sigemptyset( & set);
  sigaddset( & set, SIGUSR1);
  sigaddset( & set, SIGQUIT);

#ifdef DEBUG
  fprintf(stderr, "worker %i start living tid %d \n", i, getpid());
  fflush(stderr);
#endif

  while (1) {
    /* wait for signal */
    s = sigwait( & set, & sig);
    if (s != 0)
      continue;

#ifdef SHARED_MEMORY
    wrk->bktaskid = wrk_ptr[i].bktaskid;
    wrk->func = wrk_ptr[i].func;
    wrk->arg = wrk_ptr[i].arg;
    printf("Child     -> worker id %d ready with task id %u with arg %d\n", 
          wrk->wrkid, wrk->bktaskid, *(int *)wrk->arg);
#endif

#ifdef INFO
    fprintf(stderr, "Child     -> worker wake %d up\n", i);
#endif
    /* Busy running */
    if (wrk -> func != NULL)
    {  
      printf("\nChild     -> DO TASK...\n");
      wrk -> func(wrk -> arg);
    }
    else
      printf("Child     -> NO TASK\n");
      
    /* Advertise I DONE WORKING */
    worker[i].func = wrk_ptr[i].func;
    worker[i].arg = wrk_ptr[i].arg;
    worker[i].bktaskid = wrk_ptr[i].bktaskid;

  }
}

int bktask_assign_worker(unsigned int bktaskid, unsigned int wrkid) {
  if (wrkid < 0 || wrkid > MAX_WORKER)
    return -1;

  struct bktask_t * tsk = bktask_get_byid(bktaskid);

  if (tsk == NULL)
    return -1;

  /* Advertise I AM WORKING */
  wrkid_busy[wrkid] = 1;

  worker[wrkid].func = tsk -> func;
  worker[wrkid].arg = tsk -> arg;
  worker[wrkid].bktaskid = bktaskid;

  //printf("Assign tsk %d wrk %d with arg %d\n", worker[wrkid].bktaskid, wrkid, *(int *) worker[wrkid].arg);

  return 0;
}

int bkwrk_create_worker() {
  unsigned int i;

  for (i = 0; i < MAX_WORKER; i++) {
#ifdef WORK_THREAD
    void ** child_stack = (void ** ) malloc(STACK_SIZE);
    unsigned int wrkid = i;
    pthread_t threadid;

    sigset_t set;
    int s;

    sigemptyset( & set);
    sigaddset( & set, SIGQUIT);
    sigaddset( & set, SIGUSR1);
    sigprocmask(SIG_BLOCK, & set, NULL);
    /*
      block the SIGQUIT and SIGUSR1 signals from being delivered to the current 
      thread, so that the child thread will not be interrupted by these signals.
    */

    /* Stack grow down - start at top*/
    void * stack_top = child_stack + STACK_SIZE;

    wrkid_tid[i] = clone( & bkwrk_worker, stack_top,
      CLONE_VM | CLONE_FILES,
      (void * ) & i);

    /*
      if CLONE_VM flag is set, the calling process and child process
      run in the same memory space. 
    */
#ifdef INFO
    fprintf(stderr, "bkwrk_create_worker got worker %u\n", wrkid_tid[i]);
#endif

    usleep(100);

    #else    
    /* TODO: Implement fork version of create worker */
    pid_t pid;
    unsigned int wrkid = i;
    pid = fork();

    if (pid < 0) {
      perror("fork");
      return -1;
    } 

    if (pid == 0) {
      sigset_t set;
      int s;

      sigemptyset( & set);
      sigaddset( & set, SIGQUIT);
      sigaddset( & set, SIGUSR1);
      sigprocmask(SIG_BLOCK, & set, NULL);
      
      fprintf(stderr, "bkwrk_create_worker got worker %d\n", getpid());
      bkwrk_worker((void *)&wrkid);
      exit(0);
    } else {
      // Save the pid of the child process
      wrkid_tid[i] = pid;
    }

#endif

    usleep(100);
  }

  return 0;
}

int bkwrk_get_worker() {
  /* TODO Implement the scheduler to select the resource entity 
   * The return value is the ID of the worker which is not currently 
   * busy or wrkid_busy[1] == 0 
   */ 

  for(int i = 0; i < MAX_WORKER; i++) {
    if(wrkid_busy[i] == 0) {
      printf("Parent    -> Get worker %u\n", i);
      return i;
    }
  }

  return -1;
}

int bkwrk_dispatch_worker(unsigned int wrkid) {

#ifdef WORK_THREAD
  unsigned int tid = wrkid_tid[wrkid];

  /* Invalid task */
  if (worker[wrkid].func == NULL)
    return -1;


#ifdef DEBUG
  fprintf(stderr, "brkwrk dispatch wrkid %d - send signal %u \n", wrkid, tid);
#endif

  syscall(SYS_tkill, tid, SIG_DISPATCH);

  
  #else
  /* TODO: Implement fork version to signal worker process here */
  if (worker[wrkid].func == NULL) {
    printf("NULL");
    return -1;
  }

  // Send the SIGUSR1 signal to the process with the specified ID
  kill(wrkid_tid[wrkid], SIG_DISPATCH);

  return 0;
#endif
}